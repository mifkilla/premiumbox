...
if(is_admin()){
	...
	global $wpdb, $premiumbox;	
		...
		if(isset($_POST['id']) and is_array($_POST['id'])){
			...
			$extended = get_option('extlist_merchants');
				...
				foreach($_POST['id'] as $id){
					...
					if($id){
						if(isset($extended[$id])){
							...
							include_extanded($premiumbox, 'merchants', $id_name);
							
							if($id_name){
								do_action('ext_merchants_active_'. $id_name, $id);
								do_action('ext_merchants_active', $id_name, $id);
							}	
						}
					}	
				}	
				...
				foreach($_POST['id'] as $id){
					...
					if($id){
						if(isset($extended[$id])){
							...
							if($id_name){
								do_action('ext_merchants_deactive_'. $id_name, $id);
								do_action('ext_merchants_deactive', $id_name, $id);	
							}	
						}		
					}	
				}
				...
				foreach($_POST['id'] as $id){
					...
					if($id){
						if(isset($extended[$id])){
							...
							if($id_name){
								do_action('ext_merchants_delete_'. $id_name, $id);
								do_action('ext_merchants_delete', $id_name, $id);
							}	
						}		
					}	
				}
			...
			update_option('extlist_merchants', $extended);
			$reply = '&reply=true';
		} 
	...
	class pn_merchants_Table_List extends PremiumTable {
		...
		function column_default($item, $column_name){
			...
			if($column_name == 'title'){	
				...
				return apply_filters('merchants_settingtext_' . is_isset($item, 'script'), '<span class="bgreen">'. __('ok','pn') .'</span>', $item['name']);
			} elseif($column_name == 'security'){	
				return apply_filters('merchants_security_' . is_isset($item, 'script'), '<span class="bgreen">'. __('ok','pn') .'</span>', $item['name']);	
		...
		function get_row_actions($item){
			...
			$status = intval(is_isset($item,'status'));
			if($status == 1){
				$actions['deactive']  = '<a href="'. pn_link('pn_merchants_deactivate', 'post') . '&key=' . $item['name'] . '&_wp_http_referer=' . urlencode($_SERVER['REQUEST_URI']) .'">'. __('Deactivate','pn') .'</a>';
			} else {
				$actions['active']  = '<a href="'. pn_link('pn_merchants_activate', 'post') . '&key=' . $item['name'] . '&_wp_http_referer=' . urlencode($_SERVER['REQUEST_URI']) .'">'. __('Activate','pn') .'</a>';
			}
			$actions['edit'] = '<a href="'. admin_url('admin.php?page=pn_add_merchants&item_key='. is_isset($item, 'name')) .'">'. __('Settings','pn') .'</a>';
			return $actions;
		}			
		...
		function prepare_items(){
 		...
 		function extra_tablenav( $which ) {
			?>
			<a href="<?php echo admin_url('admin.php?page=pn_add_merchants');?>"><?php _e('Add new','pn'); ?></a>
			<?php
		} 		
	}
}