...
description: [en_US:]Garantex Crypto merchant[:en_US][ru_RU:]мерчант Garantex Crypto[:ru_RU]
version: 2.2
*/

if(!class_exists('Merchant_Premiumbox')){ return; }

if(!class_exists('merchant_garantex_crypto')){
	class merchant_garantex_crypto extends Merchant_Premiumbox {
		...
		function bidaction($temp, $m_id, $pay_sum, $item, $direction){
			...
			if($script and $script == $this->name){
				...
				if(!$to_account){
					...
					try {
						$class = new Garantex_Crypto(is_deffin($m_defin,'PRIVATE_KEY'), is_deffin($m_defin, 'UID'));
					...
					if($to_account){
						...
						set_bid_status('techpay', $item_id, $params, $direction);
						...
						$result_mail = apply_filters('premium_send_message', 0, 'generate_address2_garantexcrypto', $notify_tags, $user_send_data, get_admin_lang()); 
						...
						$result_mail = apply_filters('premium_send_message', 0, 'generate_address1_garantexcrypto', $notify_tags, $user_send_data, $bids_data->bid_locale);					
...
new merchant_garantex_crypto(__FILE__, 'Garantex Crypto');