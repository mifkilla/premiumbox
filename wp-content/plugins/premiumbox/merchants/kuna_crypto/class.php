...
if(!class_exists('Kuna_Crypto')){
	class Kuna_Crypto {
		...
		function create_address($currency, $network='', $return_url=''){
		...
		public function request($action, $data=array()){
			...
			if($ch = curl_init()){
				...
				do_action('save_merchant_error', 'kuna_crypto', 'headers: ' . print_r($headers, true) . 'post: ' . $body_string . 'result: ' . $res);
