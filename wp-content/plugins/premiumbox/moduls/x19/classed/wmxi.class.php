...
class WMXI extends WMXICore {
    ...
    public function X18($wmid, $lmi_payee_purse, $lmi_payment_no, $lmi_payment_no_type = 0, $secret_key = null) {
            #		$reqn = $this->_reqn();
            ...
            $req->lmi_payment_no_type = $lmi_payment_no_type;
            if ($this->classic) {
    ...
    public function X22( $wmid, $paymenttags, $validityperiodinhours, $secret_key = null ) {
            $req = new SimpleXMLElement('<merchant.request/>');
            $req->signtags->wmid = $wmid;
            $req->signtags->validityperiodinhours = $validityperiodinhours;
            $req->signtags->md5 = md5( $wmid.$paymenttags['LMI_PAYEE_PURSE'].$paymenttags['LMI_PAYMENT_NO'].$validityperiodinhours.$secret_key );
            foreach( $paymenttags as $sName => $sValue )
                $req->paymenttags->$sName = $sValue;
            return $this->_request( 'https://merchant.webmoney.ru/conf/xml/XMLTransSave.asp', $req->asXML(), __FUNCTION__ );
    }
    # http://wiki.webmoney.ru/wiki/show/Interfeys_X19
    public function X19($type, $direction, $pursetype, $amount, $wmid, $pnomer, $fname, $iname, $bank_name, $bank_account, $card_number, $emoney_name, $emoney_id, $phone, $crypto_name='', $crypto_address='') {
        $reqn = $this->_reqn();
		...
		if($crypto_name){
			$req->$group->crypto_name = $crypto_name;
		}
		if($crypto_address){
			$req->$group->crypto_address = $crypto_address;
		}
